...
const useMediaQuery = (queryValue, initialValue = false) => {
  ...
  useEffect(() => {
    let isMounted = true;
    const matchMedia = window.matchMedia(queryValue); //Retorna um novo objeto MediaQueryList representando o resultado analisado da string media query especificada.
...
const App = () => {
  ...
  const color = huge ? 'hotpink' : big ? 'violet' : medium ? 'pink' : small ? 'purple' : null;

  return <div style={{ fontSize: '50px', textAlign: 'center', marginTop: '20px', color }}>Hello❣</div>;
};
